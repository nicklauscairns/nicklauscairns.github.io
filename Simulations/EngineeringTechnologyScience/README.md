...
- [Spacecraft Reentry Optimization Simulation](SpacecraftReentrySimulation.html) - An interactive simulation challenging users to design a reentry vehicle by balancing mass, shield diameter, angle, and material to survive atmospheric entry against strict Mission Briefing constraints, while reflecting on model limitations.
  ...
  <details>
    ...
    <ul>
      <li>
        <b>Overview:</b> This serves as a captivating investigative phenomenon, simulating spacecraft reentry. It effectively tests students’ understanding of thermal dynamics, mass, and velocity during a critical mission phase (Criterion 4: Investigable Through Practices). It challenges students to balance engineering constraints. While aerospace engineering is specialized, the universal physics principles (friction, heat) provide strong educational value.
      </li>
      <li>
        <b>Dimensional Evaluation & Evidence Statements:</b> The simulation robustly supports the DCI (ETS1.B: Developing Possible Solutions) by utilizing a computer model to simulate a physical system. For the SEP (Using Mathematics and Computational Thinking), it strongly demonstrates Observable Features 1.a.ii (identifying the system modeled), 1.a.iii (identifying variables that can be changed: mass, angle, diameter), 2.a.i (selecting inputs), and 2.a.ii (using the model to simulate effects). It effectively fulfills 3.a by forcing students to evaluate their telemetry against a target "Mission Briefing", and fulfills 3.d by requiring explicit acknowledgement of the simulation's mathematical limitations (e.g., uniform atmosphere, simplified drag) before launch.
      </li>
      <li>
        <b>AI Action Items for Improvement:</b>
        <ul>
          <li><b>Improve CCC (Systems and System Models):</b> The simulation effectively communicates limitations and verifies expected results. However, it could be enhanced by visualizing the dynamic forces (e.g., drawing explicit vectors for gravity vs. drag on the capsule) during the descent to make the system interactions even more apparent.</li>
        </ul>
      </li>
      <li>
        <b>Implementation Checklist for AI Agent:</b>
        <ul>
          <li>[x] Add a "Mission Briefing" UI card outlining target safety limits (Max G-force: 15.0 G, Max Temp: 2000&deg;C).</li>
          <li>[x] Add an "Assumptions & Limitations" info modal listing the physical simplifications used in the mathematical model.</li>
          <li>[x] Add a prompt after the simulation runs for students to evaluate if their design "Met" or "Failed" the mission criteria compared to expected results.</li>
          <li>[ ] (Future) Add vector arrows indicating Gravity and Drag forces on the capsule during the animation.</li>
        </ul>
      </li>
    </ul>
  </details>
