...
*   **JavaScript Syntax Limitations:**
    *   Generally, prefer ES6 template literals (`` `hsla(${value})` ``) for readability and fewer escaping issues. However, explicitly use string concatenation or careful escaping in specific rewriting edge cases: (a) when embedding JS inside manipulated HTML where template literals might contain `</script>` tags that break HTML parsers, (b) when targeting legacy minifiers/transpilers lacking ES6 support, or (c) when using non-JS-aware rewriting tools (like Python scripts) that might treat `${...}` as their own template delimiters or struggle with backticks.
