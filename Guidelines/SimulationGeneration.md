...
*   **JavaScript Syntax Limitations:**
    *   **Prefer template literals by default** for normal JavaScript code (`` `hsla(${value})` ``) as they improve readability and reduce escaping errors. However, use standard string concatenation (e.g., `'hsla(' + value + ')'`) in these specific edge cases: (a) when embedding JavaScript inside manipulated HTML where template literals can hide or contain a literal `</script>` tag that some HTML parsers may break on, (b) when targeting legacy minifiers or transpilers that lack ES6 template literal support, or (c) when using non-JS-aware rewriting tools that treat `${...}` as their own template delimiter.
